...
{
    grpc::Status status{};
    auto signup_request{std::make_unique<flashback::SignUpRequest>()};
    auto signup_response{std::make_unique<flashback::SignUpResponse>()};
...
TEST_F(test_server, SignIn)
{
    ...
    user_after_signin->set_password(m_user->password());
    ...
    EXPECT_FALSE(user_after_signin->password().empty()) << "Password will be used internally by sign up function but will not return ";
    ...
    EXPECT_CALL(*m_mock_database, create_session(m_user->id(), testing::A<std::string_view>(), m_user->device())).Times(1).
