from fedora:latest
run dnf install -y bash g++ ninja wget tar gz boost-devel grpc-devel libcurl-devel protobuf-compiler npm python3 git libsodium-devel libpq-devel inotify-tools
...
run cmake --build /build --parallel 2 --target flashbackd
