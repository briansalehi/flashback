...
TEST_F(test_database, get_section_cards)
{
    ...
    std::vector<flashback::SectionCard> cards{};
    EXPECT_NO_THROW(cards = m_database->get_section_cards(resource.id(), section.position()));
    EXPECT_THAT(cards, testing::SizeIs(1));
    ASSERT_NO_THROW(cards.at(0).card().id());
    EXPECT_THAT(cards.at(0).card().id(), Eq(card.id()));
}
...
TEST_F(test_database, edit_card_headline)
{
    ...
    std::vector<flashback::Card> topic_cards{};
    std::vector<flashback::SectionCard> section_cards{};
    ...
    ASSERT_NO_THROW(section_cards = m_database->get_section_cards(resource.id(), section.position()));
    ASSERT_THAT(section_cards, testing::SizeIs(3));
    ...
    ASSERT_NO_THROW(topic_cards = m_database->get_topic_cards(subject.id(), topic.position(), topic.level()));
    ASSERT_THAT(topic_cards, testing::SizeIs(3));
    auto iter = std::ranges::find_if(topic_cards, [&second_card](flashback::Card const& tc) { return tc.id() == second_card.id(); });
    ASSERT_THAT(iter, Ne(topic_cards.cend()));
...
TEST_F(test_database, remove_card)
{
    ...
    ASSERT_NO_THROW(topic_cards = m_database->get_topic_cards(subject.id(), topic.position(), topic.level()));
    ASSERT_THAT(topic_cards, testing::SizeIs(2));
    ASSERT_NO_THROW(section_cards = m_database->get_section_cards(resource.id(), section.position()));
...
TEST_F(test_database, merge_cards)
{
    ...
    ASSERT_NO_THROW(section_cards = m_database->get_section_cards(resource.id(), section.position()));
    ASSERT_THAT(section_cards, testing::SizeIs(2));
    auto iter = std::ranges::find_if(section_cards, [&third_card](flashback::SectionCard const& sc) { return sc.card().id() == third_card.id(); });
    ASSERT_THAT(iter, Ne(section_cards.cend()));
    ASSERT_NO_THROW(iter->card().id());
    ASSERT_THAT(iter->card().id(), Eq(third_card.id()));
    EXPECT_THAT(iter->card().headline(), Eq(third_card.headline()));
}
...
TEST_F(test_database, move_card_to_topic)
{
    ...
    std::vector<flashback::Card> topic_cards{};
    std::vector<flashback::SectionCard> section_cards{};
    ASSERT_NO_THROW(topic_cards = m_database->get_topic_cards(first_subject.id(), first_topic.position(), first_topic.level()));
    ASSERT_THAT(topic_cards, testing::SizeIs(1));
    ASSERT_NO_THROW(topic_cards = m_database->get_topic_cards(second_subject.id(), second_topic.position(), second_topic.level()));
    ASSERT_THAT(topic_cards, testing::SizeIs(1));
    ASSERT_NO_THROW(topic_cards = m_database->get_topic_cards(second_subject.id(), third_topic.position(), third_topic.level()));
    ASSERT_THAT(topic_cards, testing::SizeIs(1));
    ASSERT_NO_THROW(section_cards = m_database->get_section_cards(resource.id(), first_section.position()));
    ASSERT_THAT(section_cards, testing::SizeIs(2));
    ASSERT_NO_THROW(section_cards = m_database->get_section_cards(resource.id(), second_section.position()));
    ...
    EXPECT_NO_THROW(
        m_database->move_card_to_topic(first_card.id(), first_subject.id(), first_topic.position(), first_topic.level(), second_subject.id(), second_topic.position(), second_topic.
            level()));
    ASSERT_NO_THROW(topic_cards = m_database->get_topic_cards(first_subject.id(), first_topic.position(), first_topic.level()));
    ASSERT_THAT(topic_cards, testing::IsEmpty());
    ASSERT_NO_THROW(topic_cards = m_database->get_topic_cards(second_subject.id(), second_topic.position(), second_topic.level()));
    ASSERT_THAT(topic_cards, testing::SizeIs(2));
    ASSERT_NO_THROW(topic_cards = m_database->get_topic_cards(second_subject.id(), third_topic.position(), third_topic.level()));
    ASSERT_THAT(topic_cards, testing::SizeIs(1));
...
TEST_F(test_database, make_progress)
{
    ...
    ASSERT_NO_THROW(m_database->add_card_to_topic(card.id(), subject.id(), topic.position(), topic.level()));
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), topic.level(), card.id(), duration.count()));
}
...
TEST_F(test_database, get_practice_mode)
{
    ...
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), topic.level(), first_card.id(), duration.count()));
    EXPECT_NO_THROW(mode = m_database->get_practice_mode(m_user->id(), subject.id(), milestone_level));
    ...
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), topic.level(), second_card.id(), duration.count()));
    EXPECT_NO_THROW(mode = m_database->get_practice_mode(m_user->id(), subject.id(), milestone_level));
...
TEST_F(test_database, get_practice_mode_from_high_levels)
{
    ...
    EXPECT_THAT(mode, Eq(flashback::practice_mode::aggressive));
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), first_topic.level(), first_card.id(), duration.count()));
    EXPECT_NO_THROW(mode = m_database->get_practice_mode(m_user->id(), subject.id(), flashback::expertise_level::surface));
    EXPECT_THAT(mode, Eq(flashback::practice_mode::progressive));
    EXPECT_NO_THROW(mode = m_database->get_practice_mode(m_user->id(), subject.id(), flashback::expertise_level::depth));
    EXPECT_THAT(mode, Eq(flashback::practice_mode::aggressive));
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), second_topic.level(), second_card.id(), duration.count()));
    EXPECT_NO_THROW(mode = m_database->get_practice_mode(m_user->id(), subject.id(), flashback::expertise_level::surface));
    EXPECT_THAT(mode, Eq(flashback::practice_mode::progressive));
    EXPECT_NO_THROW(mode = m_database->get_practice_mode(m_user->id(), subject.id(), flashback::expertise_level::depth));
    EXPECT_THAT(mode, Eq(flashback::practice_mode::progressive));
...
TEST_F(test_database, get_practice_mode_after_new_card_available)
{
    ...
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), topic.level(), third_card.id(), duration.count()));
    EXPECT_NO_THROW(mode = m_database->get_practice_mode(m_user->id(), subject.id(), milestone_level));
...
TEST_F(test_database, get_user_cognitive_level)
{
    ...
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), third_topic.level(), third_card.id(), duration.count()));
    ASSERT_NO_THROW(mode = m_database->get_practice_mode(m_user->id(), subject.id(), flashback::expertise_level::surface));
    EXPECT_THAT(mode, Eq(flashback::practice_mode::progressive));
    ASSERT_NO_THROW(mode = m_database->get_practice_mode(m_user->id(), subject.id(), flashback::expertise_level::depth));
    EXPECT_THAT(mode, Eq(flashback::practice_mode::progressive));
...
TEST_F(test_database, get_practice_topics)
{
    ...
    EXPECT_NO_THROW(topics = m_database->get_practice_topics(m_user->id(), roadmap.id(), milestone.id(), milestone.level()));
    EXPECT_THAT(topics, SizeIs(1));
...
TEST_F(test_database, get_study_resources)
{
    ...
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), topic.level(), first_card.id(), 20));
    EXPECT_NO_THROW(studying_resources = m_database->get_study_resources(m_user->id()));
...
TEST_F(test_database, mark_card_as_reviewed)
{
    ...
    ASSERT_NO_THROW(cards.at(0).card().id());
    ASSERT_THAT(cards.at(0).card().id(), card.id());
    ASSERT_THAT(cards.at(0).card().state(), flashback::Card::draft);
    EXPECT_NO_THROW(m_database->mark_card_as_reviewed(card.id()));
    EXPECT_NO_THROW(cards = m_database->get_section_cards(resource.id(), section.position()));
    EXPECT_THAT(cards, SizeIs(1));
    ASSERT_NO_THROW(cards.at(0).card().id());
    EXPECT_THAT(cards.at(0).card().id(), card.id());
    EXPECT_THAT(cards.at(0).card().state(), flashback::Card::reviewed);
}
...
TEST_F(test_database, mark_card_as_completed)
{
    ...
    ASSERT_NO_THROW(cards.at(0).card().id());
    EXPECT_THAT(cards.at(0).card().id(), card.id());
    EXPECT_THAT(cards.at(0).card().state(), flashback::Card::draft);
    EXPECT_NO_THROW(m_database->mark_card_as_reviewed(card.id()));
    EXPECT_NO_THROW(m_database->mark_card_as_completed(card.id()));
    EXPECT_NO_THROW(cards = m_database->get_section_cards(resource.id(), section.position()));
    EXPECT_THAT(cards, SizeIs(1));
    ASSERT_NO_THROW(cards.at(0).card().id());
    EXPECT_THAT(cards.at(0).card().id(), card.id());
    EXPECT_THAT(cards.at(0).card().state(), flashback::Card::completed);
    EXPECT_NO_THROW(m_database->mark_card_as_reviewed(card.id()));
    EXPECT_NO_THROW(cards = m_database->get_section_cards(resource.id(), section.position()));
    EXPECT_THAT(cards, SizeIs(1));
    ASSERT_NO_THROW(cards.at(0).card().id());
...
TEST_F(test_database, mark_card_as_approved)
{
    ...
    ASSERT_NO_THROW(cards.at(0).card().id());
    EXPECT_THAT(cards.at(0).card().id(), card.id());
    EXPECT_THAT(cards.at(0).card().state(), flashback::Card::approved);
}
...
TEST_F(test_database, mark_card_as_released)
{
    ...
    ASSERT_NO_THROW(cards.at(0).card().id());
    EXPECT_THAT(cards.at(0).card().id(), card.id());
    EXPECT_THAT(cards.at(0).card().state(), flashback::Card::released);
}
...
TEST_F(test_database, get_progress_weight)
{
    ...
    ASSERT_THAT(mode, Eq(flashback::practice_mode::aggressive));
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), topic.level(), first_card.id(), 20));
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), topic.level(), second_card.id(), 20));
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), topic.level(), third_card.id(), 20));
...
TEST_F(test_database, create_assessment)
{
    ...
    ASSERT_NO_THROW(mode = m_database->get_practice_mode(m_user->id(), subject.id(), milestone.level()));
    ASSERT_THAT(mode, Eq(flashback::practice_mode::aggressive));
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), first_topic.level(), first_card.id(), 20));
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), second_topic.level(), second_card.id(), 20));
    EXPECT_NO_THROW(m_database->make_progress(m_user->id(), subject.id(), third_topic.level(), third_card.id(), 20));
